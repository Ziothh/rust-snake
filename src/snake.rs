...
use crate::helpers::draw::draw_block;
...
impl Snake {
    ...
    pub fn draw(&self, ctx: &Context, g2d: &mut G2d) {
        for block in &self.body {
            draw_block(SNAKE_COLOR, block.x, block.y, ctx, g2d)
        }
    }
    ...
    pub fn move_forward(&mut self, direction: Option<Direction>) {
        ...
        let (last_x, last_y) = self.get_head_position();
    ...
    pub fn next_head(&self, direction: Option<Direction>) -> (i32, i32) {
        let (head_x, head_y) = self.get_head_position();
